...
class MediaCoverGeneratorAshan(_PluginBase):
    ...
    def api_clean_images(self):
        try:
            logger.info("【MediaCoverGeneratorAshan】收到立即清理图片缓存请求")
            self.__clean_generated_images()
            self._clean_images = False
            self.__update_config()
            return {"code": 0, "msg": "图片缓存清理完成"}
        except Exception as e:
            logger.error(f"【MediaCoverGeneratorAshan】立即清理图片失败: {e}", exc_info=True)
            return {"code": 1, "msg": f"图片缓存清理失败: {e}"}

    def api_clean_fonts(self):
        try:
            logger.info("【MediaCoverGeneratorAshan】收到立即清理字体缓存请求")
            self.__clean_downloaded_fonts()
            self._clean_fonts = False
            self.__update_config()
            return {"code": 0, "msg": "字体缓存清理完成"}
        except Exception as e:
            logger.error(f"【MediaCoverGeneratorAshan】立即清理字体失败: {e}", exc_info=True)
    ...
    def api_delete_saved_cover(self, file: str = ""):
        try:
            ...
            logger.info(f"【MediaCoverGeneratorAshan】已删除封面文件: {target_file}")
            return {"code": 0, "msg": "封面文件删除成功"}
        except Exception as e:
            logger.error(f"【MediaCoverGeneratorAshan】删除封面文件失败: {e}", exc_info=True)
    ...
    def api_generate_now(self, style: str = ""):
        ...
        try:
            if not self._enabled:
                logger.warning("【MediaCoverGeneratorAshan】立即生成失败：插件未启用，请先在设置页启用插件并保存")
                return {"code": 1, "msg": "插件未启用，请先在设置页启用插件并保存"}
            if not self._selected_servers:
                logger.warning("【MediaCoverGeneratorAshan】立即生成失败：未勾选媒体服务器，请先在设置页勾选服务器并保存")
                return {"code": 1, "msg": "未勾选媒体服务器，请先在设置页勾选服务器并保存"}
            if not self._servers:
                logger.warning("【MediaCoverGeneratorAshan】立即生成失败：服务器连接信息为空，请检查设置并保存后重试")
            ...
            logger.info(f"【MediaCoverGeneratorAshan】收到立即生成请求，风格: {self._cover_style}")
            tips = self.__update_all_libraries()
            return {"code": 0, "msg": tips or "封面生成任务已完成"}
        except Exception as e:
            logger.error(f"【MediaCoverGeneratorAshan】立即生成失败: {e}", exc_info=True)
    ...
    def api_set_cover_style(self, style: str = ""):
        try:
            ...
            self.__update_config()
            logger.info(f"【MediaCoverGeneratorAshan】已保存封面风格: {target_style}")
            return {"code": 0, "msg": f"已保存风格: {target_style}"}
        except Exception as e:
            logger.error(f"【MediaCoverGeneratorAshan】保存封面风格失败: {e}", exc_info=True)
    ...
    def api_toggle_style_variant(self):
        try:
            ...
            logger.error(f"【MediaCoverGeneratorAshan】切换静态/动态失败: {e}", exc_info=True)
    ...
    def __api_select_style(self, index: int):
        try:
            ...
            logger.error(f"【MediaCoverGeneratorAshan】选择风格失败: {e}", exc_info=True)
    ...
    def __set_page_tab(self, tab: str):
        ...
        logger.info(f"【MediaCoverGeneratorAshan】已切换页面Tab: {self._page_tab}")
    ...
    def api_saved_cover_image(self, file: str = ""):
        ...
        try:
            ...
            try:
                ...
                logger.error(f"【MediaCoverGeneratorAshan】返回图片失败: {e}")
    ...
    def get_service(self) -> List[Dict[str, Any]]:
        ...
        services.append({
            "id": "StopMediaCoverGeneratorAshan",
            "name": "停止当前更新任务",
            "trigger": None,
            "func": self.stop_task,
            "kwargs": {}
        ...
        if self._page_tab == "history-tab":
            ...
            if recent_covers:
                for item in recent_covers:
                    delete_api = f"plugin/MediaCoverGeneratorAshan/delete_saved_cover?file={quote(item['path'])}"
                    cover_rows.append(
                        {
                            "component": "VCol",
                        ...
                        "content": [
                            {
                                ...
                                "text": "封面生成",
                                "events": {"click": {"api": "plugin/MediaCoverGeneratorAshan/set_page_tab_generate", "method": "post"}},
                            },
                            {
                                "component": "VTab",
                                "props": {"value": "history-tab"},
                                "text": "历史封面",
                                "events": {"click": {"api": "plugin/MediaCoverGeneratorAshan/set_page_tab_history", "method": "post"}},
                            },
                            {
                                "component": "VTab",
                                "props": {"value": "clean-tab"},
                                "text": "清理缓存",
                                "events": {"click": {"api": "plugin/MediaCoverGeneratorAshan/set_page_tab_clean", "method": "post"}},
                            },
                        ],
                    },
                    {"component": "VDivider"},
                    ...
                    "content": [
                                    {
                                        ...
                                        "content": [
                                            ...
                                            "content": [
                                                    ...
                                                    "events": {"click": {"api": "plugin/MediaCoverGeneratorAshan/toggle_style_variant", "method": "post"}},
                                                },
                                                            {
                                                                "component": "VBtn",
                                                                "props": {
                                                    ...
                                                    "text": "立即生成当前风格",
                                                    "events": {"click": {"api": "plugin/MediaCoverGeneratorAshan/generate_now", "method": "post"}},
                                                },
                                                {
                                                    "component": "div",
                    ...
                    "content": [
                        {
                            ...
                            "content": [
                                    ...
                                    "text": "立即清理图片缓存",
                                    "events": {"click": {"api": "plugin/MediaCoverGeneratorAshan/clean_images", "method": "post"}},
                                },
                                            {
                                                "component": "VBtn",
                                                "props": {
                                    ...
                                    "text": "立即清理字体缓存",
                                    "events": {"click": {"api": "plugin/MediaCoverGeneratorAshan/clean_fonts", "method": "post"}},
                                },
                                {
                                    "component": "div",
                    ...
                    "content": [
                        {
                            ...
                            "events": {
                                "click": {
                                    "api": f"plugin/MediaCoverGeneratorAshan/select_style_{style['index']}",
                                    "method": "post",
                                }
                            },
                            "content": [
